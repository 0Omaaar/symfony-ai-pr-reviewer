...
class OAuthController extends AbstractController
{
    ...
    public function installGithubApp(): Response
    {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');

        $installUrl = $_ENV['GITHUB_APP_INSTALL_URL'] ?? '';
        if (!is_string($installUrl) || $installUrl === '') {
            return $this->json([
                'ok' => false,
    ...
    ): Response
    {
        ...
        if ($installationId > 0) {
            ...
            if (!$installation) {
                $installation = (new GithubInstallation())->setInstallationId($installationId);
    ...
    private function hydrateInstallationAccountFields(
        ...
        try {
            $response = $httpClient->request('GET', sprintf('https://api.github.com/app/installations/%d', $installationId), [
                'headers' => [
